...
public class StudentController {
	...
	public String feesSummary(Model model, @RequestParam("scholarNumber") String scholarNumber,
	...
	public String feesReportGenerate(Model model, @RequestParam("startDate") Date startDate,
	...
	public String feesProcessing(Model model, @RequestParam("scholarNumber") String scholarNumber,
			HttpServletRequest request) {
		...
		feesDao.amountToDB(scholarNumber, amount, date, paymentMode, bankName, chequeNo, chequeDate, accNo, recBank,
				remark);
	...
	public String searchByBranch(@RequestParam("branch") String branch, @RequestParam("classes") String stuClasses,
		...
		if (search.length() == 0) {
			...
			List<AdmissionDto> studentsList = studentsDao.searchStudent(stuClasses, branch, session, pageid,
					totalRecords);
			...
			List<AdmissionDto> studentsList = studentsDao.searchStudentbyScholarNumber(stuClasses, searchSch, branch,
					session);
...
//		List<String> scholarList = studentsDao.getListOfScholarNumbers();
	...
	public String saveAdmissison(AdmissionDto dto, Model model, @RequestParam("aadharPic") MultipartFile aadharPic,
		...
		System.out.println("Aadhar : " + dto.getAadhar());
		if (dto.getId() == 0) {
